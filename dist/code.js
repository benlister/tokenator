                ...
                for (const library of libraries) {
                    if (library.type === "VARIABLES") {
                        try {
                            ...
                            yield library.loadAsync();
                            if (typeof figma.variables.getVariableCollectionsFromLibraryAsync === 'function') {
                                const collections = yield figma.variables.getVariableCollectionsFromLibraryAsync(library);
                                // Process collections
...
function getSpacingVariables() {
    return __awaiter(this, void 0, void 0, function* () {
        const collections = yield getAllVariableCollections();
        const spacingTokens = [];
            ...
            try {
                if (collection.libraryName) {
                    ...
                    if (typeof figma.variables.getVariablesFromLibraryCollectionAsync === 'function') {
                        variables = yield figma.variables.getVariablesFromLibraryCollectionAsync(collection);
                    }
                    else {
                        continue;
                    }
                    ...
                    if (typeof figma.variables.getVariablesByCollectionIdAsync === 'function') {
                        variables = yield figma.variables.getVariablesByCollectionIdAsync(collection.id);
                    }
                    else {
                        continue;
                    }
...
function findExactSpacingToken(value, spacingTokens) {
    if (!spacingTokens || !spacingTokens.length)
        return null;
...
function findSpacingIssues() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const spacingTokens = hasVariablesAPI ? yield getSpacingVariables() : [];
        const issues = [];
                ...
                if (issue.type === "spacing") {
                    if (hasVariablesAPI) {
                        ...
                        if (tokenToUse && tokenToUse.id) {
                            ...
                            yield issue.node.setBoundVariable(issue.property, {
                                id: tokenToUse.id,
                                type: 'VARIABLE'
                            });
                            fixedCount++;
...
function clearMarkers() {
    try {
        const markers = figma.currentPage.findAll(node => node.type === "ELLIPSE" &&
